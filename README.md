...
- **Local artifact policy:** `data/` is generated by the pipeline and ignored by git; regenerate it with the data scripts instead of treating local CSVs as maintained source.

### Reference Documents
Three hydrogeology PDFs are available for future RAG integration:
- `a-glossary-of-hydrogeology.pdf`
- `age-dating-young-groundwater.pdf`
- `resources/pdfs/*.pdf` - Hydrogeology reference documents

The generated ChromaDB store lives in `knowledge_base/` and is ignored by git.
...
│
├── 🧠 knowledge_base/        # Generated/local ChromaDB vector store, ignored by git
│
