...
class TestFetchSingleSite:
    ...
    def test_fetch_retries_on_error(self, mock_get):
        ...
        mock_response.json.return_value = {
            "value": {
                "timeSeries": [
                    {"values": [{"value": [{"value": "15.2", "dateTime": "2023-01-01T00:00:00"}]}]}
                ]
            }
        }
        mock_response.raise_for_status.return_value = None
...
class TestPipelineOrchestration:
    ...
    def test_run_pipeline_error_resilience(self, mock_fetch, sample_valid_dataframe, temp_data_dir):
        ...
        with patch("src.data.pipeline.DATA_DIR", temp_data_dir):
            results = run_pipeline(sites=["251241080385301", "999999999999999"], parallel=False)
...
class TestGetAllSiteIds:
    ...
    def test_loads_all_configured_sites(self):
        """Should load every site listed in the current configuration."""
        site_ids = get_all_site_ids()
        expected_count = sum(
            len(aquifer.get("sites", [])) for aquifer in USGS_CONFIG.get("aquifers", {}).values()
        )

        assert len(site_ids) == expected_count
