...
class AquiferProperties:
    ...
    def validate_water_level(self, elevation_ft: float) -> Tuple[bool, str]:
        ...
        if elevation_ft < min_el - 50:  # Allow 50 ft buffer
            return False, f"Elevation {elevation_ft} ft below range {min_el}-{max_el} ft"
        if elevation_ft > max_el + 50:
            return False, f"Elevation {elevation_ft} ft above range {min_el}-{max_el} ft"
    ...
    def detect_seasonal_pattern(
        ...
        if "date" in df.columns:
            ...
            if date_span < min_years:
                logger.warning(f"Insufficient data span: {date_span:.1f} years < {min_years} year(s)")
                return None
        else:
            return None
        ...
        monthly_stats = df_copy.groupby("month")["elevation_ft"].agg(["mean", "std", "count"])
        ...
        cv = monthly_stats["std"].mean() / df_copy["elevation_ft"].mean()  # Coefficient of variation
            ...
            for idx, is_anomaly in enumerate(anomaly_mask):
                if is_anomaly and "date" in df.columns:
                    ...
                    if value > mean + 2 * std:
                        ...
                        anomaly_type = "drop"
                        cause = "Unusual low value (possibly drought or excessive pumping)"
                    else:
                        continue
