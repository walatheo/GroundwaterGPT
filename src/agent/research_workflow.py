    ...
    def research(self, query: str, session_id: str | None = None) -> dict:
        ...
        try:
            ...
            result = {
                ...
                "query": query,
                "research_plan": (
                    context.research_plan.to_dict() if context.research_plan else None
                ),
                ...
                "quality_metrics": {
                    "final_confidence": (
                        context.reflection_results[-1].confidence_score
                        if context.reflection_results
                        else 0.0
                    ),
                    "final_coverage": (
                        context.reflection_results[-1].coverage_score
                        if context.reflection_results
                        else 0.0
                    ),
                },
            ...
            self._update_progress(
                context, f"✅ Research complete ({context.total_elapsed_time:.1f}s)"
            )
            return result
