...
class TestMultiSourceSearchEngine:
    ...
    def test_kb_search_budget_recording(self):
        ...
        mock_kb_searcher = Mock(
            return_value=[{"title": "KB Doc", "snippet": "Content", "url": "kb://doc1"}]
        )
...
class TestBatchSearch:
    ...
    def test_batch_search_multiple_queries(self):
        ...
        queries = [
            SearchQuery(
                "query1", priority=0.9, source_type=SearchSourceType.KNOWLEDGE_BASE
            ),
            SearchQuery(
                "query2", priority=0.7, source_type=SearchSourceType.KNOWLEDGE_BASE
            ),
        ]
