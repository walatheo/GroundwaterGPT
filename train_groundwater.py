...
def load_groundwater_data() -> pd.DataFrame:
    ...
    print(f"  Water level range: {daily['water_level'].min():.2f} to {daily['water_level'].max():.2f} ft")
...
def create_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for window in ROLLING_WINDOWS:
        ...
        data[f'level_roll_mean_{window}d'] = data['water_level'].shift(base_shift).rolling(window).mean()
        data[f'level_roll_std_{window}d'] = data['water_level'].shift(base_shift).rolling(window).std()
        data[f'level_roll_min_{window}d'] = data['water_level'].shift(base_shift).rolling(window).min()
        data[f'level_roll_max_{window}d'] = data['water_level'].shift(base_shift).rolling(window).max()
    
    # === CHANGE FEATURES (Momentum) ===
    # Changes computed from data available FORECAST_HORIZON days ago
    data['change_7d'] = data['water_level'].shift(FORECAST_HORIZON) - data['water_level'].shift(FORECAST_HORIZON + 7)
    data['change_14d'] = data['water_level'].shift(FORECAST_HORIZON) - data['water_level'].shift(FORECAST_HORIZON + 14)
    data['change_30d'] = data['water_level'].shift(FORECAST_HORIZON) - data['water_level'].shift(FORECAST_HORIZON + 30)
...
def prepare_data(df: pd.DataFrame) -> Tuple:
    ...
    print(f"Train: {len(X_train)} samples ({dates_train.min().date()} to {dates_train.max().date()})")
...
def compare_models(X_train, y_train, X_test, y_test, feature_names: list) -> Tuple[Any, pd.DataFrame]:
...
def plot_predictions(y_test, y_pred, dates_test, model_name: str):
    ...
    fig.suptitle(f'Groundwater Level Predictions - {model_name.replace("_", " ").title()}', 
