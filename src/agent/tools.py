...
{plan.get("hypothesis", "TBD")}. The workflow centers on auditable data analysis, explicit
tracking of configurations and metrics, and reproducible reporting. Language-model assistance,
when used, is limited to communicating evidence-backed findings rather than generating new
unsupported conclusions. {key_findings}

## Introduction
Public hydrologic records are abundant but often difficult for non-specialists to use directly.
This draft focuses on a researcher-facing application that organizes groundwater records into
interpretable analyses, visual summaries, and reproducible experimental logs. The novelty is
the workflow architecture and audit trail rather than the presence of a language model alone.
...
## Discussion
The results indicate opportunities to improve researcher access to public groundwater data by
standardizing experiment plans, metadata, visual summaries, and evidence-backed reporting.
...
## Conclusion
This work presents a reproducible groundwater research workflow that emphasizes transparent data
analysis and auditability, with language-model assistance constrained to a secondary
communication role.
"""
